...
class WalletOpenApiSmokeTests {
    ...
    @Test void documentsDeletedQueryOperations() throws Exception {
        ...
        assertThat(findParameter(operation.path("parameters"), "filter").path("in").asText()).isEqualTo("query");
        JsonNode response = operation.path("responses").path("200");
        JsonNode schema = response.path("content").path("*/*").path("schema");
        assertThat(schema.path("$ref").asText("")).contains("WalletTombstonePageResponse");
    }
    ...
    @Test void documentsDeletedByIdOperation() throws Exception {
        ...
        assertThat(operation.path("responses").path("200").path("content").path("*/*").path("schema").path("$ref").asText(""))
            .contains("WalletTombstoneResponse");
        assertThat(operation.path("responses").has("404")).isTrue();
    }
    ...
    private JsonNode deletedQueryOperation(JsonNode document) {
        assertThat(document.path("paths").has("/wallets/deleted")).isTrue();
        return document.path("paths").path("/wallets/deleted").path("get");
    }

    private JsonNode deletedByIdOperation(JsonNode document) {
        assertThat(document.path("paths").has("/wallets/deleted/{id}")).isTrue();
        return document.path("paths").path("/wallets/deleted/{id}").path("get");
    }

    private JsonNode restoreOperation(JsonNode document) {
        assertThat(document.path("paths").has("/wallets/{id}/restore")).isTrue();
        return document.path("paths").path("/wallets/{id}/restore").path("post");
    }
