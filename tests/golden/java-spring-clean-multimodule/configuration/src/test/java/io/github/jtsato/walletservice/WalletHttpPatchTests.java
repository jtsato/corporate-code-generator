...
class WalletHttpPatchTests {
    ...
    void shouldPatchAndPersistWallet() throws Exception {
        ...
        HttpResponse<String> response = patch("/wallets/" + WALLET_ID, "{\"balance\":124.45,\"currency\":\"sample-2\"}");
    ...
    void shouldRejectNullRequiredField() throws Exception {
        assertThat(patch("/wallets/" + WALLET_ID, "{\"balance\":null,\"currency\":\"sample-2\"}").statusCode()).isEqualTo(400);
    }

    @Test
    void shouldPreserveOmittedbalance() throws Exception {
        walletRepository.saveAndFlush(new WalletEntity(
            WALLET_ID,
            WALLET_BALANCE,
            WALLET_CURRENCY
        ));
        HttpResponse<String> response = patch("/wallets/" + WALLET_ID, "{\"currency\":\"sample-2\"}");
        assertThat(response.statusCode()).isEqualTo(200);
        var persisted = walletRepository.findById(WALLET_ID).orElseThrow();
    ...
    void shouldReturnNotFoundForMissingWallet() throws Exception {
        assertThat(patch("/wallets/" + UUID.fromString("11111111-1111-1111-1111-111111111112"), "{\"balance\":124.45,\"currency\":\"sample-2\"}").statusCode()).isEqualTo(404);
    }
    ...
    void shouldRejectInvalidPathIdentifier() throws Exception {
        assertThat(patch("/wallets/not-a-uuid", "{\"balance\":124.45,\"currency\":\"sample-2\"}").statusCode()).isEqualTo(400);
    }
