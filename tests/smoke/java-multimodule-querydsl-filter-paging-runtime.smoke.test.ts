...
describe("Java multi-module Querydsl filter paging runtime smoke test", () => {
  it("resolves FilterExpression and PageRequest through persistence when Maven is available", async ({ skip }) => {
    await expect(access(cliEntryPoint)).resolves.toBeUndefined();
    const maven = await detectMaven(repoRoot);
    if (!maven.available) {
      const message = "Maven Querydsl filter paging runtime smoke skipped: Maven executable was not found. Set CODEGEN_REQUIRE_MAVEN_SMOKE=true to require Maven.";
      if (process.env.CODEGEN_REQUIRE_MAVEN_SMOKE === "true") throw new Error(message);
      console.warn(message); skip(message); return;
    }
    const outputRoot = await mkdtemp(join(tmpdir(), "ccg-querydsl-filter-paging-runtime-smoke-"));
    try {
      ...
      await testWithMaven(outputRoot, "*QuerydslFilterPagingPersistenceTests,*FindWalletsByFilterPageUseCaseInteractorTests");
