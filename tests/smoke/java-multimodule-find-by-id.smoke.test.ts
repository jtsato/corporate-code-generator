...
describe("Java multi-module find-by-id smoke test", () => {
  it("resolves an entity by identifier through the complete Core, persistence and HTTP paths when Maven is available", async ({ skip }) => {
    await expect(access(cliEntryPoint)).resolves.toBeUndefined();
    const maven = await detectMaven(repoRoot);
    if (!maven.available) {
      const message = "Maven find-by-id smoke skipped: Maven executable was not found. Set CODEGEN_REQUIRE_MAVEN_SMOKE=true to require Maven.";
      if (process.env.CODEGEN_REQUIRE_MAVEN_SMOKE === "true") throw new Error(message);
      console.warn(message); skip(message); return;
    }
    const outputRoot = await mkdtemp(join(tmpdir(), "ccg-multimodule-find-by-id-smoke-"));
    try {
      ...
      await testWithMaven(outputRoot, "*FindByIdPersistenceTests,*HttpFindByIdTests,*FindWalletByIdUseCaseInteractorTests");
