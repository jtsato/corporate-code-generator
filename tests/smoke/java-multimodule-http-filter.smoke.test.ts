...
describe("Java multi-module HTTP filter smoke test", () => {
  it("resolves REST filter query parameters through the complete HTTP-to-database path when Maven is available", async ({ skip }) => {
    await expect(access(cliEntryPoint)).resolves.toBeUndefined();
    const maven = await detectMaven(repoRoot);
    if (!maven.available) {
      const message = "Maven HTTP filter smoke skipped: Maven executable was not found. Set CODEGEN_REQUIRE_MAVEN_SMOKE=true to require Maven.";
      if (process.env.CODEGEN_REQUIRE_MAVEN_SMOKE === "true") throw new Error(message);
      console.warn(message); skip(message); return;
    }
    const outputRoot = await mkdtemp(join(tmpdir(), "ccg-multimodule-http-filter-smoke-"));
    try {
      ...
      await testWithMaven(outputRoot, "*HttpFilterTests");
