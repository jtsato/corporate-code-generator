...
describe("JavaSpringCleanMultimoduleConfigurationArtifactProducer", () => {
  it("produces the root application followed by deterministic domain wiring", () => {
    ...
    expect(artifacts.find((artifact) => artifact.templateId === "configuration-http-create-test")).toMatchObject({
      model: { className: "WalletHttpCreateTests", endpointPath: "/wallets", entityType: "WalletEntity", repositoryType: "WalletRepository" },
    });
  });
