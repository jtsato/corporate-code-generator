...
describe("GenerateCommand", () => {
  ...
  it.each([
    ...
    async ({ dryRun, outputDirectory }) => {
      ...
      try {
        ...
        if (!dryRun) {
          expect(writer.mock.calls[0]?.[0].operations).toHaveLength(99);
        }
      } finally {
        error.mockRestore();
      }
  ...
  it("generates the thirty-four core artifacts when the multi-module core capability is requested", async () => {
    let targetPaths: readonly string[] = [];
    const writer = vi.fn(async (plan: { readonly operations: readonly { readonly targetPath: string }[] }) => {
      targetPaths = plan.operations.map((operation) => operation.targetPath);
    });
  ...
  it("combines supported multi-module build and core capabilities in module order", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(targetPaths).toHaveLength(40);
  ...
  it.each([
    ...
    { moduleIds: ["configuration"], operationCount: 99, expectedPath: ".github/workflows/java-ci.yml" },
    { moduleIds: ["build", "configuration"], operationCount: 99, expectedPath: ".github/workflows/java-ci.yml" },
  ])(
