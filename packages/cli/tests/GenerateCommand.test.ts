...
describe("GenerateCommand", () => {
  ...
  it.each([
    ...
    async ({ dryRun, outputDirectory }) => {
      ...
      try {
        ...
        if (!dryRun) {
          expect(writer.mock.calls[0]?.[0].operations).toHaveLength(19);
        }
      } finally {
        error.mockRestore();
      }
  ...
  it.each([
    ...
    { moduleIds: ["configuration"], operationCount: 14, expectedPath: "configuration/src/test/java/io/github/jtsato/walletservice/WalletHttpSmokeTests.java" },
    { moduleIds: ["build", "configuration"], operationCount: 19, expectedPath: "infra/database/pom.xml" },
  ])(
