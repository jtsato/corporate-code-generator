...
`smoke:multimodule:nestjs` is the layout gate for the multi-module profile ([ADR-095](../adr/ADR-095-nestjs-multimodule-variant.md)). It requires no external toolchain and is included in the default `npm test` run. The multi-module profile is a layout variant rather than a capability set, so the gate compares it against the single-package profile rather than only against its own golden: every artifact the single-package profile emits must appear under the declared path rewrite, and the only additions may be the workspace scaffolding. A variant that quietly dropped an artifact would still build and still match its own golden; only the cross-profile comparison catches it. It also asserts that a cross-layer import is a package name where the other layout uses a relative path, that each package's `dependencies` and TypeScript `references` state the dependency direction, and that Jest's `moduleNameMapper` and `tsconfig.spec.json` agree about resolving package names to sources.

`smoke:generated-project-multimodule:nestjs` is the execution gate for that profile. It generates, installs, lints, builds with `tsc --build`, runs both suites, then starts the compiled composition root and asserts the same REST contract — including soft delete, the deleted-only routes and restore, each of which crosses a package boundary. It additionally asserts that npm linked every workspace package and that each compiled into its own `dist` with declarations, because a workspace can generate perfectly and still fail to link, and a build can succeed while the server dies on its first require. It proves the generated boundary lint is not vacuous by adding an inward-crossing package import to a Core model and requiring the lint to reject it. It requires npm registry access, is excluded from `npm test`, and runs as its own step.
...
For milestone 7.29, final evidence is complete: `npm run typecheck` and `npm run build` exited 0; `npm test` passed with 61 files and 365 tests; `npm run smoke:multimodule:nestjs` passed with 1 file and 8 tests; and `$env:CODEGEN_REQUIRE_NPM_SMOKE='true'; npm run smoke:generated-project-multimodule:nestjs` passed with 1 file and 3 tests. The single-package goldens came out byte-identical to the ones 7.28 approved, which is what shows that parameterizing 136 cross-layer imports changed nothing for that profile. The generated workspace was installed, linted, built and run, passing 79 unit plus 5 end-to-end tests in memory and 87 plus 5 under TypeORM. All four combinations of model and persistence option generate byte-identically across two runs under the multi-module profile as well.

The layout assertions were proven non-vacuous by relocating one inherited artifact to a path the declared rewrite does not predict, which failed exactly the three cases that compare the two profiles and the golden; the manifest was then restored. Dropping a manifest entry outright cannot pass silently either, because the producer still invokes it and generation fails.
...
| Shared NestJS template touched by both layouts | Typecheck, build, `npm run smoke:nestjs`, `npm run smoke:multimodule:nestjs` | `npm run smoke:generated-project-multimodule:nestjs` with `CODEGEN_REQUIRE_NPM_SMOKE=true` when generated build or runtime behavior changes. A template with a cross-layer import must take a layer root with the single-package relative path as its default, or it will render a relative path into a workspace. |
