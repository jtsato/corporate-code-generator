...
The implemented `java-spring-clean-multimodule` profile baseline is 93 artifacts (build 6, Core 34, entrypoints-rest 45, Infra 52, Configuration 93). The build+core selection is 40 artifacts and build+configuration is 93. Selection counts for `entrypoints-rest` and `infra-database` include Core transitively, since both declare `requires: [core]`. Its baseline capabilities are:
