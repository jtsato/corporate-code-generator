...
| NestJS package-backed i18n | Supported in NestJS Golden Path | Generated `nestjs-i18n` 10.6.0 JSON catalogs use `AcceptLanguageResolver`, English fallback, and Portuguese selection for language tags beginning with `pt`; Core retains only a framework-free i18n contract. |
...
| NestJS uniqueness | Supported in NestJS Golden Path | In-memory persistence rejects duplicate identifiers, `unique: true` values, and complete non-null `uniqueGroups` tuples with HTTP 409; update/PATCH ignores the current record. Soft-delete reuse is not implied because NestJS DELETE remains physical. |
