...
The multi-module Golden Path has 70 artifacts. Its build module generates a minimal GitHub Actions Java CI workflow using Java 25, Maven cache, and `mvn -B clean verify`. Core Filter Common provides validated filter operators, conditions, groups, and expressions. The REST Filter Contract Foundation in `entrypoint.rest.common.filter`, with passive per-domain definitions in `entrypoint.rest.domains.<domain>.filter`, parses the future HTTP contract into `FilterExpression`. `WalletController` does not yet use this parser: `GET /wallets`, OpenAPI, and Querydsl runtime behavior are unchanged.

The future flow is `REST query params -> RestFilterParser -> FilterExpression -> Querydsl mapper -> repository runtime`; only the first transformation exists now. `infra.database.domains.<entity>.query` continues to supply passive Querydsl predicate builders; Q-types are generated by Maven and never committed.
