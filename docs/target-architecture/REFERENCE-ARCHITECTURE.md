...
The multi-module Golden Path has 63 artifacts. Its build module generates a minimal GitHub Actions Java CI workflow using Java 25, Maven cache, and `mvn -B clean verify`. Sonar, JaCoCo, PIT, Testcontainers, and Docker remain future capabilities. Core Filter Common provides validated filter operators, conditions, groups, and expressions without REST or persistence integration. `infra.database.domains.<entity>.query` supplies passive Querydsl predicate builders; Q-types are generated by Maven and never committed. REST/runtime filter integration remains future scope.
