...
The multi-module Golden Path has 88 artifacts (build 6, Core 31, entrypoints-rest 41, Infra 49, and Configuration 88). Because `entrypoints-rest` and `infra-database` require `core`, their selection counts include Core transitively; their own production remains 10 and 18. Its build module generates a minimal GitHub Actions Java CI workflow using Java 25, Maven cache, and `mvn -B clean verify`. Core Filter Common provides validated filter operators, conditions, groups, and expressions. The REST Filter Contract Foundation in `entrypoint.rest.common.filter`, with per-domain definitions in `entrypoint.rest.domains.<domain>.filter`, parses the HTTP contract into `FilterExpression`. Infra also generates entity-aware Querydsl filter definitions from each entity's actual attributes and Java types.
...
REST query parameters now reach that same runtime: `HTTP query params -> RestFilterParser -> FilterExpression -> Find<Entity>ByFilterUseCase -> QuerydslFilterMapper -> ListQuerydslPredicateExecutor -> JPA/H2 -> <Entity>Response`. `WalletController` injects `FindWalletsByFilterUseCase` (not the untouched `FindWalletsUseCase`, which stays generated and registered), binds a repeatable `filter` query parameter (`@RequestParam(name = "filter", required = false) List<String>`), and only orchestrates `RestFilterParser.parse` plus `WalletRestFilterDefinition.create()` — no parsing, alias, or operator logic is duplicated in the controller. A generated `RestFilterWebConfiguration` (`configuration.web`) registers a `String -> List<String>` `Converter` so that Spring's default request-parameter binding does not comma-split a single `filter` occurrence (which would otherwise corrupt the `in` operator's own comma-separated value list); repeated `filter` occurrences are unaffected and still combine with AND. The parameter is documented in OpenAPI via `@Parameter`/`@ArraySchema` on the controller method. Both `RestFilterParser` and `QuerydslFilterMapper` report errors as the existing `ValidationException`, converted to HTTP 400 by the unchanged `GlobalExceptionHandler` — no new exception type or handler was introduced. `infra.database.domains.<entity>.query` continues to supply passive Querydsl predicate builders; Q-types are generated by Maven and never committed. Paging, sorting, and OR/nested REST syntax remain future work; see [ADR-037](../adr/ADR-037-rest-filter-runtime-integration.md).

## Core paging

`core.common.paging` provides Spring-free `PageRequest`, `PageResult`, `SortDirection`, and `SortOrder`. Paging validation uses `ValidationException`. The Spring Data adapter (`SpringDataPageRequestMapper`, `SpringDataPageResultMapper`) is now wired to the persistence runtime through a dedicated paginated use case, separate from `Find<Entity>UseCase` and `Find<Entity>ByFilterUseCase`, both of which stay untouched. The generated flow is `PageRequest -> Find<Entity>PageUseCase -> <Entity>Gateway.findPage -> SpringDataPageRequestMapper.toPageable(pageRequest, Map.of()) -> JpaRepository.findAll(Pageable) -> SpringDataPageResultMapper.toPageResult -> PageResult<Entity>`. `Map.of()` is a deliberate, already-tested choice: sorting stays out of scope for this milestone, `PageRequest.of(page, size)` already produces an empty `sort` list, and no per-entity sort-property allowlist is generated yet. Null rejection lives in the interactor as a `ValidationException` keyed `common.paging.page-request.required` — the same key the mapper already used for the same concept — while the provider states only an `Objects.requireNonNull` precondition. Configuration registers the paginated use case as an explicit fourth bean. Pagination is not yet reachable over HTTP: `GET /wallets` still returns `List<WalletResponse>`, with no `page`/`size`/`sort` query parameters and no OpenAPI documentation for them; see [ADR-038](../adr/ADR-038-paging-runtime-integration.md).
