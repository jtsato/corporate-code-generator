...
The multi-module Golden Path has 55 artifacts. Its build module generates a minimal GitHub Actions Java CI workflow using Java 25, Maven cache, and `mvn -B clean verify`. Sonar, JaCoCo, PIT, Testcontainers, and Docker remain future capabilities. `infra.database.domains.<entity>.query` supplies passive Querydsl predicate builders; Q-types are generated by Maven and never committed. REST/runtime integration remains future scope.
