...
constructs the domain entity through `Create<Entity>UseCase`, and reaches
`JpaRepository.existsById` and `save` through the persistence provider and
mapper. Successful creation returns HTTP 201, a relative `Location`, and the
...
> create runtime is exercised through Core, H2 persistence, and the generated
> REST HTTP create test; update and delete remain outside the capability.
